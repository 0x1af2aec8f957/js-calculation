import { hasNumericValue, add, subtract, multiply, divide, mod, pow } from 'mathjs';
...
export function isConstantNumber(token: string) { // 是否是Math携带的常量
    ...
    return Object.hasOwnProperty.call(Math, token) && typeof Math[token] === 'number';
}

export function isFunction(token: string){ // 是否是Math携带的方法
    return Object.hasOwnProperty.call(Math, token) && typeof Math[token] === 'function';
}
